...
class Request:
    ...
    def announce_request(self, torrent, peerID, downloaded, uploaded, left, event, ip, port):
        ...
        ip_list = ip.split(".")
        data[72:73] = (int(ip_list[0])).to_bytes(1, "little")
        data[73:74] = (int(ip_list[1])).to_bytes(1, "little")
        data[74:75] = (int(ip_list[2])).to_bytes(1, "little")
        data[75:76] = (int(ip_list[3])).to_bytes(1, "little")
